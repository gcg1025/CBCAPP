...
@interface NewsClassContentV () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
@implementation NewsClassContentV
    ...
    if (self.allBlock) {
        if (self.isAllSelect) {
            ...
            for (NSInteger i = self.selectModel.subClass.count - 1; i >= 0; i --) {
                NewsClassM *model = self.selectModel.subClass[i];
                if (ary.count > 1) {
